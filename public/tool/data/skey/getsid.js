...
function login(uin,pwd){
	...
	var param = {uin: uin, pwd: pwd, p: p, vcode: vcode, pt_verifysession: pt_verifysession, cookie: cookie};
...
function dovc(uin,code,vc){
	...
	var param = {uin: uin, ans: code, sig: vc, cap_cd: cap_cd, sess: sess, collectname: collectname, websig: websig, cdata: cdata, sid: sid};
	xiha.postData(getvcurl, param, function(d) {
		if(d.rcode == 0){
			var pwd=$('#pwd').val();
...
function checkvc(){
	...
	var param = {uin: uin, tokenid: tokenid};
	xiha.postData(getvcurl, param, function(d) {
		if(d.saveOK ==0){
			$('#uin').attr('cookie',d.cookie);
