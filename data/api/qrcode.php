...
if (!$text || !$C_Frame || !$C_Frame) {
    ...
    echo '{"code":501,"message":"fail:参数缺失"}';
} else {
    //判断参数是否正确
    if ($size < 50 || $size > 800) {
        header('Content-type: application/json');
        echo '{"code":501,"message":"fail:size超出范围"}';
        exit;
    } else if ($C_Frame < 1 || $C_Frame > 10) {
        header('Content-type: application/json');
        echo '{"code":501,"message":"fail:frame超出范围"}';
        exit;
    } else if ($C_E != 'L' & $C_E != 'M' & $C_E != 'Q' & $C_E != 'H') {
        header('Content-type: application/json');
        echo '{"code":501,"message":"e fail:参数错误"}';
        exit;
    }
    //生成二维码
    if ($C_E = 'L') {
