...
/* 图片引入及处理 */
$bigImgPath = __DIR__ . "/res/60s/yes.jpg";
$qCodePath = __DIR__ . "/res/60s/" . date("w") . ".jpg";

$bigImg = imagecreatefromstring(file_get_contents($bigImgPath));
$qCodeImg = imagecreatefromstring(file_get_contents($qCodePath));
list($qCodeWidth, $qCodeHight, $qCodeType) = getimagesize($qCodePath);
imagecopymerge($bigImg, $qCodeImg, 40, 40, 0, 0, $qCodeWidth, $qCodeHight, 100);
...
$text = "来自 {$area} 的朋友";
$text .= "\n当前时间:" . date("Y年m月d日 h时i分s秒") . " " . $weekList[date('w')];
...
$im = imagecreatetruecolor(980, 1860);
imagecopy($im, $bigImg, 0, 0, 0, 0, 980, 1860);
imagedestroy($bg);
$black = imagecolorallocate($im, 00, 00, 00);
$black1 = imagecolorallocate($im, 209, 209, 209);
imagettftext($im, 20, 0, 40, 610, $black, $font, $text);
imagettftext($im,25,0,630,1840, $black1, $font,"HULICore");
imagejpeg($im);
imagedestroy($im);
...
function getHistory()
{
	...
	$aa = '';
	for ($int = count($event[1]); $int > 0 & $int > count($event[1]) - 20; $int--) {
		$aa = $aa . $event[1][$int - 1] . "\n";
	}
	return $aa;
}
...
class Lunar
{
     ...
     * 将阳历转换为阴历
     * @param int $year 公历-年
     * @param number $month 公历-月
     * @param number $date 公历-日
     */
